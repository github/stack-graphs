...
use stack_graphs::c::sg_path_list_count;
use stack_graphs::c::sg_path_list_free;
use stack_graphs::c::sg_path_list_new;
...
fn check_jump_to_definition(graph: &TestGraph, expected_paths: &[&str]) {
    ...
    let results = unsafe {
        std::slice::from_raw_parts(
            sg_path_list_paths(path_list) as *const Path,
            sg_path_list_count(path_list),
        )
    };
    let results = results
        .iter()
