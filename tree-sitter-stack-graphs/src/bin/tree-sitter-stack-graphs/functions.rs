...
pub fn add_path_functions(ctx: &mut Context, functions: &mut Functions) {
    functions.add(ctx.add_identifier("normalize-path"), path::NormalizePath);
    functions.add(ctx.add_identifier("resolve-path"), path::ResolvePath);
}
...
pub mod path {
    ...
    impl Function for NormalizePath {
        fn call(
            ...
            let path = parameters.param()?.into_string(graph)?;
            parameters.finish()?;
    ...
    impl Function for ResolvePath {
        fn call(
            ...
            let base_path = parameters.param()?.into_string(graph)?;
            let path = parameters.param()?.into_string(graph)?;
            parameters.finish()?;
