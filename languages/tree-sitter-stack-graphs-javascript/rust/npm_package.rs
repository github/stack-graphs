...
impl FileAnalyzer for NpmPackageAnalyzer {
    fn build_stack_graph_into<'a>(
        ...
        let pkg_internal_name_pop =
            add_pop(graph, file, pkg_internal_guard_pop, pkg_internal_name, "");

        // reach package internals via root
        //
        //     [push pkg_internal_name] -> [push "GUARD:PKG_INTERNAL"] -> [root]
        //
        let pkg_internal_guard_push = add_push(graph, file, root, PKG_INTERNAL_GUARD, "");
        let pkg_internal_name_push =
            add_push(graph, file, pkg_internal_guard_push, pkg_internal_name, "");
        ...
        if !npm_pkg.name.is_empty() {
            ...
            //
            let pkg_guard_pop = add_pop(graph, file, root, PKG_GUARD, "");
            let pkg_name_pop =
                add_module_pops(graph, file, Path::new(&npm_pkg.name), pkg_guard_pop, "");
        ...
        //
        let dep_guard_pop = add_pop(graph, file, pkg_internal_name_pop, PKG_GUARD, "");
        let dep_guard_push = add_push(graph, file, root, PKG_GUARD, "");
        for (_, (dep_name, _)) in npm_pkg.dependencies.iter().enumerate() {
            if dep_name.is_empty() {
                continue;
            }
            let dep_name_pop = add_module_pops(graph, file, Path::new(dep_name), dep_guard_pop, "");
            let dep_name_push =
                add_module_pushes(graph, file, Path::new(dep_name), dep_guard_push, "");
